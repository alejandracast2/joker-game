...
export class ReelContainer extends Container {
  ...
  // ANTES: private reel: Sprite;
  // AHORA: separa el wrapper (Container) del sprite de fondo
  private reelWrap: Container;
  private reelSprite: Sprite;
  ...
  constructor(bgW: number, bgH: number) {
    ...
    // 🎰 Wrapper contenedor (sí puede tener hijos)
    this.reelWrap = new Container();
    this.addChild(this.reelWrap);

    // 🎰 Sprite de fondo (NO tendrá hijos)
    const reelTexture = Texture.from("carrete1.png");
    this.reelSprite = new Sprite(reelTexture);
    this.reelSprite.anchor.set(0.5);
    this.reelWrap.addChild(this.reelSprite);

    // 💡 Textura compartida
    const lightsTexture = Texture.from("transparencia.png");

    // 🔼 Luces superiores (hijas del WRAPPER, no del sprite)
    this.topLights = new Sprite(lightsTexture);
    this.topLights.anchor.set(0.5);
    this.topLights.blendMode = "add"; // evita strings en v8
    this.topLights.alpha = 0.6;
    this.reelWrap.addChildAt(this.topLights,1);
    this.topLights.y = -this.reelSprite.height / 2 + 33;
    this.topLights.x = 12;
    ...
    this.bottomLights.alpha = 0.4;
    this.bottomLights.scale.y = -1;
    this.reelWrap.addChild(this.bottomLights);
    this.bottomLights.y = this.reelSprite.height / 2 - 33;
    this.bottomLights.x = 12;

    // Raíz de slots y contenedor de reels (también cuelgan del WRAPPER)
    this.slotsRoot = new Container();
    this.reelWrap.addChild(this.slotsRoot);

    this.reelContainer = new Container();
    this.slotsRoot.addChild(this.reelContainer);

    // === setup de medidas ===
    const VIEW_WIDTH = 1435;
    const VIEW_HEIGHT = 530;
    const SYMBOL_SIZE = Math.floor(VIEW_HEIGHT / this.ROWS);
    const REEL_WIDTH  = Math.floor(VIEW_WIDTH / this.COLS);

    // Alineación de reelContainer respecto al sprite de fondo
    this.reelContainer.x = (-this.reelSprite.width / 2);
    this.reelContainer.y = (-this.reelSprite.height / 2);

    // === MÁSCARA (v8) -> hija del WRAPPER, no del sprite ===
    const clip = new Graphics().rect(0, 0, VIEW_WIDTH, VIEW_HEIGHT).fill(0xffffff);
    clip.position.copyFrom(this.reelContainer);
    clip.eventMode = "none";
    this.reelWrap.addChild(clip);
    this.reelContainer.mask = clip;

    // === crear reels ===
    const slotTextures = [
      "perla-0.png","gema-0.png","zapatos-0.png","diamante-0.png",
      "bolos-0.png","guitar-0.png","joker-0.png","bonus-0.png"
    ].map(Texture.from);
    ...
    // Offset del wrapper (centra todo el conjunto)
    this.reelOffset = {
  ...
  resize() {
    ...
    this.reelWrap.position.set(ox, oy);
  }
}
...
//     const VIEW_HEIGHT = 530;      // alto visible del área de símbolos por columna
...
// // Posiciona la máscara en el mismo origen que reelContainer
// clip.position.copyFrom(this.reelContainer);

// // Para que no intercepte eventos de puntero (v8 usa eventMode)
// clip.eventMode = "none";

// // La máscara debe estar en el árbol de display
// this.reel.addChild(clip);

// // Aplica la máscara SOLO al área de los símbolos
// this.reelContainer.mask = clip;
